...
def conditional_gradient(f, grad_f, linear_minimizer, x0, max_iter=1000, tol=1e-6, verbose=False):
    ...
    for i in range(max_iter):
        ...
        s = linear_minimizer(gradient)
        
        gap = np.dot(gradient, x - s)
        
        if verbose and (i+1) % 100 == 0:
            print(f"Итерация {i+1}: f(x) = {f_val:.6f}, gap = {gap:.6f}")
...
if __name__ == "__main__":
    ...
    def linear_minimizer_unit_circle(grad):
    ...
    x_opt_, f_opt_, history = conditional_gradient(
        f, grad_f, linear_minimizer_unit_circle, x0,
        max_iter=1000,
        tol=1e-4,
        verbose=True
    )
    ...
    print(f"Оптимальная точка (преобр.): x_ = [{x_opt_[0]:.6f}, {x_opt_[1]:.6f}, {x_opt_[2]:.6f}]")
    print(f"Значение функции (преобр.): f_(x_) = {f_opt_:.6f}")
    print(f"Норма точки: ||x_|| = {np.linalg.norm(x_opt_):.6f}")
    print(f"Оптимальная точка: x = [{x_opt[0]:.6f}, {x_opt[1]:.6f}, {x_opt[2]:.6f}]")
    print(f"Значение исходной функции: f(x) = {f_opt:.6f}")
    print(f"Количество итераций: {len(history)-1}")

    fig = visualize_3d_trajectory_plotly(history)
    
    fig.write_html("cond.html")
